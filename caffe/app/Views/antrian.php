    ...
    <div class="col-lg-6 grid-margin stretch-card">
        <div class="card">
            <div style="text-align: center;" class="card-body">
                ...
                <p class="card-description">
                    Pesanan pelanggan yang sudah diterima hari ini.
                </p>
                <div class="table-responsive">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
                    ...
                    <div class="col-6">
                        <div class="form-group">
                            <div class="input-group">
                                ...
                                <input type="text" id="noMeja" class="form-control" aria-label="Amount (to the nearest dollar)">
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-warning" onclick="proses()" id="proses">Pesanan Telah Sampai</button>
            </div>
        </div>
    </div>
</div>
...
<script>
            ...
            success: function(data) {
                if (data.length) {
                    for (let i = 0; i < data.length; i++) {
                        ...
                        if (data[i].status == 0) {
                            isiPesanan += "<label class='badge badge-danger'>Mengantar Pesanan"
                        } else {
                            isiPesanan += "<label class='badge badge-success'>Proses Pembayaran"
                        }
    ...
    function tampilkanAntrianSelesai() {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.length) {
                    for (let i = 0; i < data.length; i++) {
                        isiPesanan += "<tr><td>" + data[i].noMeja + "</td><td>" + data[i].nama + "</td><td><label class='badge badge-success'>SUKSES</label></td><td><button href='#' class='btn btn-inverse-success btn-sm' onClick='modalRincian(" + data[i].id + ", \"" + data[i].nama + "\", " + data[i].noMeja + "," + data[i].status + ")'><i class='mdi mdi-playlist-check'></i><i class='mdi mdi-food'></i></button></td></tr>"
                    }
                } else {
    ...
    function modalRincian(id, nama, noMeja, status) {
        ...
        tampilkanRincian(id)
        if (status == 0) {
            $("#proses").html("Pesanan Telah Sampai")
        } else if (status == 1) {
            $("#proses").html("Pembayaran Selesai")
        } else {
            $("#proses").hide()
        }
